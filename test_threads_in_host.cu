...
void launchKernel(float2 *H, float2 *hY, int K, int N, int blockSize, int gridSize, int nrOfRunns, cudaStream_t stream){
	...
	//	mtx.unlock();
		...
		hermitian_transpose<<<blockDims,GridDims, 0, stream>>>(dH, dHH,K,N);

		//Number of threads are N*N
		Ltriangle_complex_matrix_mult<<<blockDims,GridDims, 0, stream>>>(dHH, dH, dmHH,N,K,N);	
		...
		for(int i = 0; i < N; i++){
			...
			bChol2<<<blockSize,1, 0, stream>>>(dmHH,i,N);
			//cudaDeviceSynchronize();
			cudaStreamSynchronize(stream);
			//Part3 of cholesky and start cInv part1
			cInv1<<<blockSize,1, 0, stream>>>(dmHH,dInv,i,N);
			
			bChol3<<<blockDims,GridDims, 0, stream>>>(dmHH,i,N);
			cudaStreamSynchronize(stream);//cudaDeviceSynchronize();
			//Part2 of inv
			cInv2<<<blockDims,GridDims, 0, stream>>>(dmHH,dInv,i,N);
			//printf("\n");
		}
		
		//This part takes the inv of L multiplied with itsef to become A^-1
		hermitian_transpose<<<blockDims,GridDims, 0, stream>>>(dInv, dInvH,N,N);
		cudaStreamSynchronize(stream);//cudaDeviceSynchronize();
		//complex_matrix_mult<<<blockDims,GridDims>>>(dInv, dInvH, dInvM,K,K,K); Right way but not for Ali
		complex_matrix_mult<<<blockDims,GridDims, 0, stream>>>(dInvH, dInv, dInvM,N,N,N);
		
		//dHH = 8x128 dy = 128x1 dHHY = 8x1
		complex_matrix_mult<<<blockDims,GridDims, 0, stream>>>(dHH, dY, dHHY,N,K,1);
		cudaStreamSynchronize(stream);//cudaDeviceSynchronize();
		//dHH = 8x8 dHHY = 8x1
		complex_matrix_mult<<<blockDims,GridDims, 0, stream>>>(dInvM, dHHY, dx,N,N,1);
...
int main() {
	...
	int nrOfRunns = 1; //only for one so that the thread timing is right!!!!
	...
	//timing before launch threads and after join
	cudaEvent_t start, stop;     		// using cuda events to measure time
	float elapsed_time_ms;       		// which is applicable for asynchronous code also
	cudaEventCreate(&start);     		// instrument code to measure start time
	cudaEventCreate(&stop);
    ...
    std::vector<cudaStream_t> streams(nrOfFrames);
    ...
    // Launch multiple host threads, each running the 'launchKernel' function
    for (int i = 0; i < nrOfFrames; ++i) {
        threads.emplace_back(launchKernel, H, hY, K, N, blockSize, gridSize, nrOfRunns, streams[i]);//(float2 *H, float2 *hY, int K, int N, int blockSize, int gridSize, int nrOfRunns
    }
	...
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&elapsed_time_ms, start, stop );
	printf("total time before launch threads to join: %f\n", elapsed_time_ms);
