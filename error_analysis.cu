...
float2 *read_matrix_from_csv(char filename[], int num_rows, int num_cols) {
	...
	for(int i=0;i<2;i++){
	  ...
	  char line[8192*2];
	  int row = 0, col = 0;
	  while (fgets(line, 8192*2, file) && row < num_rows) {
...
int main() {
	...
	for(int o=0; o<nrOfRunns;o++){
		...
		for(int frame = 0; frame<nrOfFrames; frame++){
			...
			cudaMalloc((void **)&dHHY[frame], N*sizeof(cuFloatComplex));
			cudaMalloc((void **)&dx[frame], N*sizeof(cuFloatComplex));
		}	
		...
		float2 result[N];
		memcpy(result, hHHY[0], N*sizeof(cuFloatComplex));
		float avg_error_real = 0;
		float avg_error_img = 0;
		float avg_error_tot = 0;
