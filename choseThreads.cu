...
int main() {
	...
	cuFloatComplex hInv[K*K]; //wet int varf̦r men denna m̴ste va med
	float2 hHHY[N];
	...
	printf("Time to calculate hermetian: %f ms.\n", elapsed_time_ms2);
	...
	cudaEventSynchronize(stop2);
	cudaEventElapsedTime(&elapsed_time_ms2, start2, stop2 );
	printf("Time to calculate multiplikation: %f ms.\n", elapsed_time_ms2);
