...
double2 *read_matrix_from_csv(char filename[], int num_rows, int num_cols) {
	...
	for(int i=0;i<2;i++){
	  ...
	  char line[16384];
	  int row = 0, col = 0;
	  while (fgets(line, 16384, file) && row < num_rows) {
...
*/
__device__ cuDoubleComplex cuCsqrt(cuDoubleComplex z){
	double r = cuCabs(z);
    double theta = atan2(z.y,z.x);
    cuDoubleComplex sqrt_z = make_cuDoubleComplex(sqrtf(r) * cosf(theta / 2.0f),
...
*/
__global__ void cInv2(double2* A,double2* Ainv, int i, int N){
	...
	for(int v=0;v < extraRows; v++){
		for(int w=0; w<extraCols;w++){
			...
			if(tmpRow <= rowElements && tmpCol <=N){
				...
				Ainv[tmpCol*N+tmpRow+i+1] = cuCsub(Ainv[tmpCol*N+tmpRow+i+1],cuCmul(Ainv[tmpCol*N+i],A[i*N+tmpRow+i+1]));
...
/**
   ...
   N is the nr of rows/columns of the A matrix (NxN)
*/
__global__ void cInv1(double2* A,double2* Ainv, int i, int N){
	...
	for(int v=0;v < extraRows; v++){
		...
		if(tmpCol <= elements){
			...
			Ainv[tmpCol*N+i] = cuCdiv(Ainv[tmpCol*N+i],A[i*N+i]);
		}
	}
}
...
/**
   ...
   The A matrix is overwriten in this funktion
*/
__global__ void bChol3(double2* A, int i, int N){
	...
	for(int v=0;v < extraRows; v++){
		for(int w=0; w<extraCols;w++){
			...
			if(tmpRow >= tmpCol && tmpRow<=elements && tmpCol<=elements){
				...
				double2 tmp = A[(N*i+i+1)+tmpCol];
				tmp.y = -tmp.y;
				A[(tmpCol+j)*N+j+tmpRow] = cuCsub(A[(tmpCol+j)*N+j+tmpRow],cuCmul(A[(N*i+i+1)+tmpRow],tmp));
			}
		}
	}	
}
...
/**
   ...
   The A matrix is overwriten in this funktion
*/
__global__ void bChol2(double2* A,int i,int N){
	...
	for(int v=0;v < extraRows; v++){
		...
		if(tmpRow <= elements){
			...
			A[(i*N+i)+tmpRow] = cuCdiv(A[(i*N+i)+tmpRow], A[i*N+i]);
		}
	}
}
...
/**
   ...
   The A matrix is overwriten in this funktion
*/
__global__ void bChol(double2* A,int i,int N){
...
*/
__global__ void hermitian_transpose(const double2* input_h, double2* output_hh, int K, int N) { //const because we do not want to modify the input matrix!!!
	...
	for(int v=0;v < extraRows; v++){
		for(int w=0; w<extraCols;w++){
			...
			if(tmpRow < K && tmpCol <N){
				...
				//conjugate here - in a double2: .x is the real part, .y is imaginary part
...
*/
__global__ void complex_matrix_mult(const double2* A, const double2* B, double2* C, const int res_row, const int a_col_b_row, const int res_col) {
	int rowthread = blockDim.x * gridDim.x;
	...
	for(int v=0;v < extraRows; v++){
			for(int w=0; w<extraCols;w++){
				...
				if (tmpRow < res_row && tmpCol < res_col) {		
					double2 sum = make_double2(0.0f, 0.0f);
				
				for (int k = 0; k < a_col_b_row; k++) {
					//printf("(%d,%d) a: %d   b: %d\n",row,col,row * a_col_b_row + k, k * res_col + col);
					double2 a = A[k * res_row + tmpRow]; //column-major!!!!!!
					double2 b = B[tmpCol * a_col_b_row + k];
...
/**
	...
	res_col is nr of columns in B matrix
*/
__global__ void Ltriangle_complex_matrix_mult(const double2* A, const double2* B, double2* C, const int res_row, const int a_col_b_row, const int res_col) {
...
int main() {
	...
	/*for (int i = 0; i<N; ++i) {
		for (int j = 0; j<K; ++j) {
			//printf("%d \n", i);
			printf("%f+%fi ", H[i*N+j].x,H[i*N+j].y);
		}
		printf("\n");
	}*/
	//Time stuff
	...
	for(int o=0; o<nrOfRunns;o++){
		...
		cuDoubleComplex *dH, *dHH, *dmHH, *dInv, *dInvH,*dInvM,*dY,*dHHY,*dx;
		cudaMalloc((void **)&dH, K*N*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dHH,  N*K*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dmHH, N*N*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dInv, N*N*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dInvH, N*N*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dInvM, N*N*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dY, K*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dHHY, K*sizeof(cuDoubleComplex));
		cudaMalloc((void **)&dx, K*sizeof(cuDoubleComplex));
		
		cudaEventRecord(start, 0);
		//Copy input data to array on GPU.
		cudaMemcpy(dH, H, K*N*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
		cudaMemcpy(dY, hY, K*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
		...
		//cudaMemcpy(hmHH, dmHH, N*N*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
		...
		cudaMemcpy(hHHY, dx, N*sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
		...
		for (int i = 0; i<N; ++i) {
			...
			//printf("%f+%fi \n", hHHY[i].x,hHHY[i].y);
				...
				//Calculate error rate for each element__________________________________________
		double2 errors[N];
		
		//read xhat file
		char file_x[32] = "";
		sprintf(file_x, "%dx%d/xhat_zf", K,N);
		double2 *xhat;
		xhat = read_matrix_from_csv(file_x, N, 1);
		
		double2 result[N];
		//result = hHHY[0];
		memcpy(result, &hHHY[0], N*sizeof(cuDoubleComplex));
		double avg_error_real = 0;
		double avg_error_img = 0;
		double avg_error_tot = 0;
		for(int row=0; row<N; row++){//compare result to correct result
			errors[row].x = (result[row].x - xhat[row].x) / xhat[row].x;//real
			errors[row].y = (result[row].y - xhat[row].y) / xhat[row].y;//img
			avg_error_real += errors[row].x;
			avg_error_img += errors[row].y;
			avg_error_tot += errors[row].x+errors[row].y;;
			printf("result: %f %fi xhat: %f %fi error: real: %f img: %f\n", result[row].x, result[row].y, xhat[row].x, xhat[row].y, errors[row].x, errors[row].y);
		}
		printf("avg real: 	%f avg img: 	%f avg both: 	%f\n", avg_error_real/N, avg_error_img/N, avg_error_tot/(2*N));
